...
RUN_GROUP="age1_76"
RUN_NAME="600epoch_bank_resample_uniform_t1_age2"
CONFIG_PATH="${ROOT}/train_run2/${RUN_GROUP}/${RUN_NAME}/config.yaml"
...
OUT_DIR="${ROOT}/runs2/${RUN_GROUP}/${RUN_NAME}/${TIMESTAMP}"
...
export PATH="${X0LOOP_CONDA_BIN:-/root/miniconda3/envs/vl/bin}:${PATH}"
...

{
  ...
  echo "[env] torchrun=$(command -v torchrun)"
} >> "${LOG_FILE}" 2>&1
...
if [ "${X0LOOP_RUN_BACKGROUND:-1}" = "1" ]; then
  ...
  cat <<EOF
==================================================================
[x0loop] ${RUN_GROUP}/${RUN_NAME} started in background (GPU=${CUDA_VISIBLE_DEVICES}, nproc=${NPROC_PER_NODE}, pid=${RUN_PID})
  log        : ${LOG_FILE}
