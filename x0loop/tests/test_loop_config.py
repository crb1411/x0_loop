...
def test_run_steps_must_be_positive():
    cfg = _config(None)
    cfg["train"]["run_steps"] = 0
    with pytest.raises(ValueError, match="train.run_steps must be > 0"):
        build_loop_config(cfg, loader=range(5), distributed_cfg={})
