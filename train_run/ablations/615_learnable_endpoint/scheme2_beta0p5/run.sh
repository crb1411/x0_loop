...
OUT_DIR="${ROOT}/runs/ablations/cifar10_flow_x0_vloss/jit/615_learnable_endpoint/${RUN_NAME}/${TIMESTAMP}"
...

{
  ...
  echo "[torchrun] CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} MASTER_PORT=${MASTER_PORT}"
} | tee -a "${LOG_FILE}"

train_run/run_and_plot.sh "${LOG_FILE}" "${TIMESTAMP}" "${ROOT}" -- \
  torchrun \
  --nnodes=1 \
  --node_rank=0 \
  --nproc_per_node=1 \
