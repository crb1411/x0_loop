#!/usr/bin/env bash
# Usage:
#   bash train_run/ablations/cifar10_flow_x0_vloss/run_dit_logitnormal_pmean_0p8_sequential.sh [extra x0loop.train args...]
#
# Runs DiT logit-normal P_mean=0.8/P_std=0.8 ablations one by one:
#   train_dit_logitnormal_pmean_0p8*.sh
set -euo pipefail
...
collect_scripts() {
  local path
  for path in "${DIR}"/train_dit_logitnormal_pmean_0p8*.sh; do
    [ -e "${path}" ] || continue
    basename "${path}"
  done | sort
}
...
if [ "${#SCRIPTS[@]}" -eq 0 ]; then
  echo "[ablation-suite] no matching scripts: ${DIR}/train_dit_logitnormal_pmean_0p8*.sh" >&2
  exit 1
fi
