#!/usr/bin/env bash
# Usage:
#   bash train_run/train_x0loop_cifar10_oneclick.sh [ckpt.pt] [extra x0loop.train args...]
# Logs:
#   runs/launcher_logs/train_x0loop_cifar10_oneclick/<timestamp>/logs/launcher.log
...
LOG_DIR="runs/launcher_logs/train_x0loop_cifar10_oneclick/${X0LOOP_RUN_TIMESTAMP}/logs"
mkdir -p "${LOG_DIR}"
...
setsid train_run/run_and_plot.sh "${LOG_FILE}" "${X0LOOP_RUN_TIMESTAMP}" "${ROOT}" -- \
  ...
  -m x0loop.train \
  --config x0loop/configs/cifar10_dit_diffusion_train.yaml \
