...
if not rows:
    print(f"no results yet under {base}"); sys.exit(0)
rows.sort(key=lambda r: (r[4] is None, r[4] if r[4] is not None else 1e9))
hdr = ("experiment", "sampler", "steps", "cfg", "FID", "IS", "KID", "P", "R")
print(f"{hdr[0]:<26}{hdr[1]:<10}{hdr[2]:>5} {hdr[3]:>4}  {hdr[4]:>8} {hdr[5]:>6} {hdr[6]:>9} {hdr[7]:>5} {hdr[8]:>5}")
def fmt(v, n): return ("%.*f" % (n, v)) if isinstance(v, (int, float)) else "-"
for r in rows:
    print(f"{r[0]:<26}{str(r[1]):<10}{str(r[2]):>5} {str(r[3]):>4}  "
