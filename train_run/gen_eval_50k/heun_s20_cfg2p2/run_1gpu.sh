#!/usr/bin/env bash
# Run a 50k FID/IS/KID eval on one GPU for heun, steps=20, cfg=2.2.
set -euo pipefail
...
echo "[gen_eval_50k] exp=${EXP} sampler=heun steps=20 cfg=2.2 samples=50000 GPU=${CUDA_VISIBLE_DEVICES}" | tee -a "${LOG_FILE}"
