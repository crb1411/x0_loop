#!/usr/bin/env bash
# Usage:
#   bash train_run/train_533/train_x0loop_cifar10_unet_weighted_x0_lossv.sh [ckpt.pt] [extra x0loop.train args...]
# Logs:
#   runs/launcher_logs/train_533_unet_x0_vloss/<timestamp>/logs/launcher.log
...
LOG_DIR="runs/launcher_logs/train_533_unet_x0_vloss/${X0LOOP_RUN_TIMESTAMP}/logs"
mkdir -p "${LOG_DIR}"
...
setsid train_run/run_and_plot.sh "${LOG_FILE}" "${X0LOOP_RUN_TIMESTAMP}" "${ROOT}" -- \
  ...
  -m x0loop.train \
  --config train_run/configs/cifar10/cifar10_unet_flow_train_x0_vloss.yaml \
