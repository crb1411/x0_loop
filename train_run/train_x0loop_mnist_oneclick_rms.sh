...
set -euo pipefail
# ============================================================
# X0-Loop MNIST one-click launcher (single node / single GPU)
# Usage:
#   bash /mnt/seek/aigc/x0_loop/runs/train_x0loop_mnist_oneclick.sh [resume_ckpt_path]
# ============================================================
...
# Train config
CONFIG_PATH="${WORK_DIR}/x0loop/configs/mnist_dit_diffusion_rms.yaml"
RUNTIME_CONFIG_PATH="${WORK_DIR}/x0loop/configs/runtime/fsdp_checkpoint_compile.yaml"
DEFAULT_RESUME_CKPT="/mnt/seek/aigc/x0_loop/runs/mnist_diffusion/run_20260213_223227/checkpoints/ckpt_step_00156000.pt"
...
export MASTER_PORT="${MASTER_PORT:-40019}"
export PYTHONPATH="${WORK_DIR}:${PYTHONPATH:-}"
export HOSTNAME="$(hostname)"

# Output directory root (each run gets its own timestamp subdir)
OUTPUT_ROOT="${WORK_DIR}/runs/mnist_diffusion"
mkdir -p "${OUTPUT_ROOT}"
...
# Build full per-run config (base + runtime + run-specific overrides).
if [[ -n "${RESUME_CKPT}" && -f "${RESUME_CKPT}" ]]; then
  ...
  if [[ -n "${RESUME_CKPT}" ]]; then
    ...
    RESUME_STATUS="从头训练（未设置resume checkpoint）"
  fi
fi
