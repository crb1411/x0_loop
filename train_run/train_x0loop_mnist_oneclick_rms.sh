#!/usr/bin/env bash
# Usage:
#   bash train_run/train_x0loop_mnist_oneclick_rms.sh [ckpt.pt] [extra x0loop.train args...]
# Logs:
#   runs/launcher_logs/train_x0loop_mnist_oneclick_rms/<timestamp>/logs/launcher.log
...
LOG_DIR="runs/launcher_logs/train_x0loop_mnist_oneclick_rms/${X0LOOP_RUN_TIMESTAMP}/logs"
mkdir -p "${LOG_DIR}"
...
setsid torchrun \
  ...
  -m x0loop.train \
  --config x0loop/configs/mnist_dit_diffusion_rms.yaml \
